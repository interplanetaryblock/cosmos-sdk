...
// NewHandler returns a handler for "naming" type messages.
func NewHandler(keeper Keeper) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) sdk.Result {
		...
		default:
			errMsg := fmt.Sprintf("Unrecognized naming Msg type: %v", reflect.TypeOf(msg).Name())
...
func handleMsgSetName(ctx sdk.Context, keeper Keeper, msg MsgSetName) sdk.Result {
	if bytes.Compare(msg.Owner, keeper.GetOwner(ctx, msg.NameID)) != 0 { // Checks if the the msg sender is the same as the current owner
